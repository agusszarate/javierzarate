  ...
  try {
    ...
    if (!body)
      return NextResponse.json(
        { success: false, message: "EMPTY_BODY" },
        { status: 400 }
      );
    ...
    if (body.mode === "byPlate" && !body.licensePlate)
      errors.push("MISSING_LICENSE_PLATE");
    if (
      body.mode === "byVehicle" &&
      (!body.vehicle?.year || !body.vehicle?.brand || !body.vehicle?.model)
    )
      errors.push("MISSING_VEHICLE_FIELDS");
    if (errors.length)
      return NextResponse.json(
        { success: false, message: "VALIDATION_ERROR", errors },
        { status: 400 }
      );

    const meridionalUrl =
      process.env.MERIDIONAL_QUOTE_URL ||
      "https://meridionalseguros.seg.ar/auto-cotizador";

    // Use environment-aware launcher to avoid Exec format errors locally
      ...
      results: [
        {
          ...
          monthly: 123456,
          currency: "ARS",
          details: "Cobertura completa",
          franchise: "30.000",
        },
        {
          planName: "Terceros Completo",
          monthly: 98765,
          currency: "ARS",
          details: "Daños a terceros, robo total",
          franchise: "—",
        },
      ],
      inputsEcho: body,
    ...
    return NextResponse.json(
      { success: false, message: "UNEXPECTED", traceId },
      { status: 500 }
    );
  }
}
